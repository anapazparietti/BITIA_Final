document.addEventListener("DOMContentLoaded", () => {
    ...
    function confirmSelection() {
        // Verificación de seguridad para loopIsVisible
        if (typeof loopIsVisible !== 'undefined' && loopIsVisible) return;

        // si vuelve a seleccionar la misma → alterna play/pause
        if (activeIndex === selectedIndex && isVisible && (typeof enDemandasApp !== 'undefined' ? enDemandasApp : true)) {
    ...
    // === CONTROL CON TECLADO (Actualizado a -, 8, 0) ===
    window.addEventListener("keydown", (event) => {
        if (event.defaultPrevented) return;

        // CAMBIO: Switch basado en event.key
        switch (event.key) {
            case "-": // Antes "Digit1" (Confirmar)
                console.log("- pressed (Confirmar Demanda)");
                if (typeof enDemandasApp !== 'undefined' && enDemandasApp) {
                    updateSelection();
                    confirmSelection();
                }
                break;

            case "8": // Antes "KeyA" (Anterior)
                // Chequeo de variables globales
                if ((typeof loopIsVisible === 'undefined' || !loopIsVisible) && (typeof enDemandasApp !== 'undefined' && enDemandasApp)) {
                    console.log("8 pressed (Anterior Demanda)");
            ...
            case "0": // Antes "KeyS" (Siguiente)
                if ((typeof loopIsVisible === 'undefined' || !loopIsVisible) && (typeof enDemandasApp !== 'undefined' && enDemandasApp)) {
                    console.log("0 pressed (Siguiente Demanda)");
